...
def test_adapter_update_avoids_expensive_calls():
    ...
    with patch.object(TankWorld, 'get_stats',  wraps=adapter._world.get_stats) as mock_get_stats:
        with patch.object(TankWorld, 'get_recent_poker_events', wraps=adapter._world.get_recent_poker_events) as mock_get_events:
            ...
            assert mock_get_events.call_count == 0, "get_recent_poker_events() was called during update()!"
...
def test_adapter_step_uses_lightweight_metrics_by_default():
    ...
    with patch.object(TankWorld, 'get_stats', return_value={}) as mock_get_stats:
        ...
        assert call_args.kwargs.get('include_distributions') is False, "step() should request lightweight metrics"
