...
def test_engine_delta_tracking(simulation_engine):
    ...
    assert any(s.reason == "test_spawn" for s in engine._frame_spawns), "Spawn not tracked in _frame_spawns"
    ...
    assert any(r.reason == "test_remove" for r in engine._frame_removals), "Removal not tracked in _frame_removals"
...
def test_delta_reset_every_frame(simulation_engine):
    ...
    assert not any(s.reason == "leaked" for s in engine._frame_spawns), "Frame deltas not cleared at start of frame"
...
def test_backend_adapter_returns_deltas(simulation_engine):
    ...
    assert any(s.reason == "contract_test" for s in result.spawns), "StepResult missing spawns from engine"
