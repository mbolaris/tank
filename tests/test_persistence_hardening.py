...
def test_persistence_round_trip():
    ...
    test_nectar = PlantNectar(
        x=200, y=200,
        source_plant=test_plant,
        environment=world
    )
    ...
    assert len(snapshot["entities"]) >= 3, f"Expected at least 3 entities, got {len(snapshot['entities'])}"
    ...
    restored_test_plant = next(
        (e for e in dest_entities if isinstance(e, Plant) and e.plant_id == 999),
        None
    )
    assert restored_test_plant is not None, "Test plant with ID 999 should be restored"
    assert abs(restored_test_plant.energy - 42.0) < 0.01, f"Plant energy mismatch: {restored_test_plant.energy}"
    ...
    test_nectar_restored = next(
        (n for n in restored_nectars if n.source_plant and n.source_plant.plant_id == 999),
        None
    )
    assert test_nectar_restored is not None, "Test nectar should be restored with link to plant 999"
    assert abs(test_nectar_restored.energy - 7.5) < 0.01, f"Nectar energy mismatch: {test_nectar_restored.energy}"
    # Most importantly: verify the reference integrity
    assert test_nectar_restored.source_plant is restored_test_plant, "Nectar.source_plant should reference the restored plant object"
    ...
    assert abs(restored_test_fish.energy - test_fish_energy) < 0.01, f"Fish energy mismatch: {restored_test_fish.energy}"
