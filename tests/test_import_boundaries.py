...
def check_forbidden_imports(
    ...
    allowed_exceptions: List[str] = None
) -> List[Tuple[Path, str]]:
...
class TestBackendBoundaries:
    ...
    def test_no_direct_tank_world_imports(self):
        ...
        filtered_violations = [
            (path, imp) for path, imp in violations
            if path.name != "world_registry.py"
        ]
