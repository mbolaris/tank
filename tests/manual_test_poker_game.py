...
def test_poker_game_creation():
    ...
    ai_fish = [
        ...
        {
            "fish_id": 3,
            "name": "PokerBluffer Gen 2",
            "energy": 110,
            "algorithm": "PokerBluffer",
            "aggression": 0.9,
        },
    ]

    game = HumanPokerGame(
        game_id="test-e2e-001", human_energy=500.0, ai_fish=ai_fish, small_blind=5.0, big_blind=10.0
    )
    ...
    for player in game.players:
        assert len(player.hole_cards) == 2, "Each player should have 2 hole cards"

    print(f"✓ Game created with {len(game.players)} players")
    print(f"✓ Initial pot: {game.pot} (from blinds)")
    print("✓ All players dealt 2 hole cards")
    ...
    while not game.game_over and actions_taken < max_actions:
        ...
        if current_player.is_human:
            ...
            if state["call_amount"] > 0:
                game.handle_action("human", "call")
                print(f"  Human calls {state['call_amount']}")
            else:
                game.handle_action("human", "check")
                print("  Human checks")
        else:
...
def test_backend_integration():
    ...
    assert result is None or result.get("success"), "start_poker should succeed"
    assert runner.human_poker_game is not None, "Should create poker game"

    state = runner.human_poker_game.get_state()
    print("✓ Poker game created via command")
    ...
    print("✓ Poker action processed successfully")
    print()
...
def test_error_handling():
    ...
    game = HumanPokerGame(
        game_id="error-test", human_energy=500.0, ai_fish=ai_fish, small_blind=5.0, big_blind=10.0
    )
    ...
    for _ in range(max_attempts):
        ...
        if current.is_human:
            ...
            if state["call_amount"] > 0:
                ...
                print("✓ Correctly rejects check when call is required")
                break
        else:
            # Let AI act
            game._next_player()
