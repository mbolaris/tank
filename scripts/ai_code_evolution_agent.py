...
"""
    ...
    def validate_improvement(
        ...
        if result.passed:
            ...
            logger.info(f"  Reproduction: {result.baseline_reproduction_rate:.2%} -> {result.new_reproduction_rate:.2%} ({result.improvement_reproduction:+.1%})")
            logger.info(f"  Survival: {result.baseline_survival_rate:.2%} -> {result.new_survival_rate:.2%} ({result.improvement_survival:+.1%})")
            logger.info(f"  Lifespan: {result.baseline_avg_lifespan:.0f} -> {result.new_avg_lifespan:.0f} ({result.improvement_lifespan:+.1%})")
            return True, None
        else:
    ...
    def run(self, stats_file: str) -> None:
        ...
        while attempts < max_attempts:
            ...
            if self.validate:
                passed, error = self.validate_improvement(
                    algo_name, improved_code, performance
                )
                if passed:
