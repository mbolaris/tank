...
def score_poker_fish(fish) -> dict:
    ...
    score = (win_rate * 0.4 + positional_balance * 0.4 +
             roi_score * 0.1 + experience * 0.1)
...
def run_and_capture(seed: int, max_frames: int = 200000):
    ...
    for frame in range(max_frames):
        ...
        if frame > 0 and frame % stats_interval == 0:
            ...
            if len(qualified) >= 3:
                ...
                logger.info(f"  → {len(qualified)} qualified fish | Best score: {best['score']:.3f} "
    ...
    logger.info("=" * 60)
    logger.info("\nNext steps:")
    logger.info(f"1. Evaluate: python3 scripts/submit_solution.py evaluate {solution.metadata.solution_id} --hands 500 --duplicates 15")
