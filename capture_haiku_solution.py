#!/usr/bin/env python3
"""Capture Haiku-4.5 poker solution from an extended simulation."""
...
def run_simulation_for_poker(max_frames: int = 75000, seed: int = 4545):
    """Run an extended simulation to evolve superior poker skills.

    Uses a longer run (75k frames) to evolve potentially better strategies
    than the Sonnet-4.5 champion (which used 50k frames).
    """
    logger.info("Starting extended simulation for Haiku-4.5...")
...
def capture_best_solution(world, author: str = "Haiku-4.5", name: str = "Haiku-4.5 Poker Master"):
    ...
    if not poker_fish:
        ...
        if fish_list:
            ...
            solution = tracker.capture_solution(
                best_fish,
                name=name,
                description=f"Haiku-4.5 solution evolved from extended 75k frame simulation",
                author=author,
            )
            return solution
        return None
    ...
    solution = tracker.capture_solution(
        best_fish,
        name=name,
        description=f"Haiku-4.5 poker strategy evolved from 75k frame simulation - {games} games played",
        author=author,
    )
...
def main():
    logger.info("=" * 60)
    logger.info("CAPTURING HAIKU-4.5 POKER SOLUTION")
    logger.info("=" * 60)
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("HAIKU-4.5 POKER MASTER CAPTURED")
    print("=" * 60)
