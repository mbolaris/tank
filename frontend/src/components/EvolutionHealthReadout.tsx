...
import { analyzeSelectionQuality } from '../utils/selectionAnalysis';
...
export function EvolutionHealthReadout({ history, onOpenTrends, compact = false, livePopulation = null }: EvolutionHealthReadoutProps) {
    ...
    const selectionAnalysis = analyzeSelectionQuality(history);
    ...
    const selectionTone = selectionAnalysis.tone === 'good' ? styles.good : selectionAnalysis.tone === 'warning' ? styles.warning : styles.neutral;
        ...
        <aside className={`${styles.readout} ${expanded ? styles.expanded : styles.collapsed}`} aria-label="Evolution health">
            <div className={styles.summaryRow}>
                <button className={styles.summaryToggle} onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
                    ...
                    <span className={styles.summary}>Selection {selectionAnalysis.selectionText} · Diversity {((last.diversity_score ?? 0) * 100).toFixed(0)}%</span>
                </button>
            ...
            <div className={styles.metrics}>
                <div className={selectionTone} title={selectionAnalysis.detailText}>
                    <span>Selection</span>
                    <strong>{selectionAnalysis.selectionText}</strong>
                </div>
