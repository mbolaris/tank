...
export function getFishPattern(
  ...
  if (params.pattern_intensity < 0.1) {
    return null; // No pattern
  }

  const width = baseSize * params.body_aspect;
  const height = baseSize;
  const opacity = params.pattern_intensity * 0.4; // Max 40% opacity
  ...
  switch (params.pattern_type) {
    ...
    case 2: // Solid (darker overlay)
      return `
        <path d="${getFishPath(params, baseSize)}" fill="${baseColor}" opacity="${opacity * 0.5}" />
      `;
