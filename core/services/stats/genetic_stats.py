...
def _get_trait_values(fish_list: List["Fish"], trait_name: str, category: str = "physical") -> List[float]:
...
def _get_lifespan_modifier_stats(fish_list: List["Fish"]) -> Dict[str, Any]:
    ...
    return _compute_numeric_stats(values, LIFESPAN_MODIFIER_MIN, LIFESPAN_MODIFIER_MAX, "lifespan_modifier")
...
def _build_gene_distributions(fish_list: List["Fish"]) -> Dict[str, Any]:
    ...
    def build_from_specs(*, category: str, traits_attr: str, specs: List[Any]) -> List[Dict[str, Any]]:
        ...
        for spec in specs:
            ...
            traits = []
            for f in fish_list:
                if not hasattr(f, "genome"): continue
    ...
    physical = build_from_specs(category="physical", traits_attr="physical", specs=PHYSICAL_TRAIT_SPECS)
    behavioral = build_from_specs(category="behavioral", traits_attr="behavioral", specs=BEHAVIORAL_TRAIT_SPECS)
    ...
    try:
        ...
        adult_sizes = []
        for f in fish_list:
            if hasattr(f, "genome") and hasattr(f.genome, "physical") and hasattr(f.genome.physical, "size_modifier"):
...
def _get_poker_strategy_distributions(fish_list: List["Fish"]) -> List[Dict[str, Any]]:
    ...
    for f in fish_list:
        if not hasattr(f, "genome"): continue
        if not hasattr(f.genome.behavioral, "poker_strategy"): continue
