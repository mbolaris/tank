...
def run_quick_eval(config: QuickEvalConfig) -> QuickEvalResult:
    ...
    Returns:
        QuickEvalResult with score, statistics, and determinism hash
    """
    ...
    for cycle in range(config.max_cycles):
        ...
        if current_touch_id and current_touch_id != last_touch_id:
            ...
            if player:
                touches[player.team] += 1
                event_log.append((cycle, "touch", player.team, current_touch_id))
        ...
        for event in step_result.get("events", []):
            if event.get("type") == "goal":
                event_log.append(
                    (cycle, "goal", event["team"], event.get("scorer_id"))
                )
    ...
    final_state = {
        ...
        "final_player_pos": [
            (pid, round(engine.get_player(pid).position.x, 6),
