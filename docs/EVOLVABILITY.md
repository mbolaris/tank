...
- **New primitive (2026‑07‑14):** `core/behavior/target_memory.py` — a domain‑neutral, evolvable target‑commitment module (memory duration, confidence decay, switch threshold, commitment strength, motion‑extrapolation duration) shared by food‑seeking and soccer‑ball pursuit, opt‑in behind `target_memory_enabled`. It answers "what target am I committed to, and when should I switch?" — the second reusable cross‑domain module after the Target Pursuit Module (`core/behavior/pursuit_nodes.py`), and the first designed from the outset to generalize to prey/predators/social partners. No graveyard entry yet — the food‑to‑soccer transfer study (compare food‑evolved vs. soccer‑evolved vs. random‑module memory on zero‑shot reacquisition) is the natural first experiment.
