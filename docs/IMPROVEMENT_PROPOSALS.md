...
`-> Any`, `[Any]`) and **800 plain `Any` occurrences** across `core/`. Both
...
`dict[str, Any]`; a plain `\bAny\b` count is 800) and replace the easy ones
...
`core/simulation/debug_trace.py`, `core/services/stats/selection_quality.py`, and
`core/genetics/trait.py`.
