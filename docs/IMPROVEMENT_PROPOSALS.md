...
`-> Any`, `[Any]`) and **789 plain `Any` occurrences** across `core/`. Both
...
`dict[str, Any]`; a plain `\bAny\b` count is 789) and replace the easy ones
...
`core/genetics/trait.py`, `core/simulation/event_managers.py`, and
`core/entities/predators.py`.
