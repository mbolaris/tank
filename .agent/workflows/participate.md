...
# Participate in Board Discussion
...
evidence - post it as a reply. Tag it `reply` and reference the comment
you're responding to with `--metric re=<comment_id>` (there's no threading in
this schema, so the metric is how a reader traces the conversation):
