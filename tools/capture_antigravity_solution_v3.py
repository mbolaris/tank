...
def run_simulation_for_poker(max_frames: int = 150000, seed: int = 12345):
    """Run an extended simulation to evolve superior poker skills.

    Uses a very long run (150k frames) and new seed 12345.
    Must be run with aggressive config modifications (ante=10.0, mutation=0.2).
    """
...
def capture_best_solution(world, author: str = "Antigravity", name: str = "Antigravity Poker Supreme"):
    ...
    solution = tracker.capture_solution(
        best_fish,
        name=name,
        description=f"Antigravity v3 poker strategy evolved from 150k frame simulation - {games} games played - Aggressive Config",
        author=author,
    )
...
def main():
    logger.info("=" * 60)
    logger.info("CAPTURING ANTIGRAVITY v3 POKER SOLUTION")
    logger.info("=" * 60)
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("ANTIGRAVITY POKER SUPREME CAPTURED")
    print("=" * 60)
