...
def run_sim(max_frames=75000, seed=2024):
    ...
    for frame in range(max_frames):
        ...
        if frame > 0 and frame % 5000 == 0:
            ...
            poker_games = sum(f.poker_stats.total_games if hasattr(f, 'poker_stats') and f.poker_stats else 0 for f in fish_list)
...
def capture_and_eval(world):
    ...
    fish_list = [e for e in world.entities_list if isinstance(e, Fish)]
    poker_fish = [(f, f.poker_stats.total_games) for f in fish_list if hasattr(f, 'poker_stats') and f.poker_stats and f.poker_stats.total_games > 0]
    if not poker_fish:
        if fish_list:
    ...
    solution = tracker.capture_solution(best_fish, name="Haiku-4.5 Poker Pro", description="75k frames seed 2024", author="Haiku-4.5")

    config = SolutionBenchmarkConfig(hands_per_opponent=200, num_duplicate_sets=10,
        opponents=["always_fold", "random", "loose_passive", "tight_passive", "tight_aggressive", "loose_aggressive", "maniac", "balanced"])
    ...
    logger.info(f"SEED 2024: Elo {result.elo_rating:.0f}, Tier {result.skill_tier}, bb/100 {result.weighted_bb_per_100:+.2f}")
